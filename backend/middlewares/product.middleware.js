...
verifyBarcode = (req, res, next) => {
    product_model.findOne({barcode: req.body.barcode}).exec((error, product) =>{
        if(error) return res.status(500).json({error: true, mensaje: error.details[0].message});
        if(product) return res.status(400).json({error: true, mensaje: product.description + "ya está registrado con el código " + product.barcode})
        next();
    });
};
